...
import { soundManager } from '@/lib/soundEngine';
...
export function useMultiplayerBingo() {
  ...
  useEffect(() => {
    ...
    s.on('countdown', ({ timeLeft }) => {
      ...
      if (timeLeft > 0 && timeLeft <= 5) soundManager.playTick();
    ...
    s.on('gameEnd', ({ winnerNickname, winningCards }) => {
      ...
      if (winnerNickname && winnerNickname !== 'No winner') soundManager.playBingo();
    });
